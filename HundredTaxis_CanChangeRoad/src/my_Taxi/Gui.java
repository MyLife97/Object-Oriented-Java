...
class guiInfo {
	...
	public static ArrayList<Integer> getPath(int root , int destination){
			...
			for (int i = 1; i <= 4; i++) {
				...
				if (next >= 0 && next < 6400 && graph[n.NO][next] == 1) {
					...
					if(nextn.depth == n.depth + 1){
						//compare current
						if(n.current + guigv.GetFlow(n.NO / 80, n.NO % 80, nextn.NO / 80, nextn.NO % 80) < nextn.current){
							nextn.current = n.current + guigv.GetFlow(n.NO / 80, n.NO % 80, nextn.NO / 80, nextn.NO % 80);
