...
class node {// 结点信息
	...
	int current;//车流量
	ArrayList<Integer> path = new ArrayList<Integer>();//路径
...
class guiInfo {
	...
	public void pointbfs(int root , int destination) {// 单点广度优先搜索
		...
		for (int i = 0; i < 6400; i++) {
			for(int j = 1; j <= 4; j++){
				if(i + offset[j] >= 0 && i + offset[j] < 6400){
					D[i][i + offset[j]] = graph[i][i + offset[j]];
					D[i + offset[j]][i] = graph[i + offset[j]][i];
				}
...
class TaxiGUI {// GUI接口类
	...
	public static void SetLightStatus(Point p,int status){//设置红绿灯 status 0 没有红绿灯 1 东西方向为绿灯 2 东西方向为红灯
				...
				if (di == 0) {// 在同一水平线上
					...
					if (status == 0) {// 关闭
						if (guigv.m.map[p.x][p.y] == 3) {
							guiInfo.graph[p.x*80 + p.y][p.x*80 + p.y + 1] = gv.MAXNUM;
							guiInfo.graph[p.x*80 + p.y + 1][p.x*80 + p.y] = gv.MAXNUM;
						...
						if (guigv.m.map[p.x][p.y] == 2 && Map.memmap[p.x][p.y] == 3) {
							guiInfo.graph[p.x*80 + p.y][p.x*80 + p.y + 1] = 1;
							guiInfo.graph[p.x*80 + p.y + 1][p.x*80 + p.y] = 1;
							guigv.m.map[p.x][p.y] = 3;
						} else if (guigv.m.map[p.x][p.y] == 0 && (Map.memmap[p.x][p.y] == 1 || Map.memmap[p.x][p.y] == 3)) {
							guiInfo.graph[p.x*80 + p.y][p.x*80 + p.y + 1] = 1;
							guiInfo.graph[p.x*80 + p.y + 1][p.x*80 + p.y] = 1;
					...
					if (status == 0) {// 关闭
						if (guigv.m.map[p.x][p.y] == 3) {
							guiInfo.graph[p.x*80 + p.y][p.x*80 + p.y + 80] = gv.MAXNUM;
							guiInfo.graph[p.x*80 + p.y + 80][p.x*80 + p.y] = gv.MAXNUM;
							guigv.m.map[p.x][p.y] = 1;
						...
						if (guigv.m.map[p.x][p.y] == 1 && Map.memmap[p.x][p.y] == 3) {
							guiInfo.graph[p.x*80 + p.y][p.x*80 + p.y + 80] = 1;
							guiInfo.graph[p.x*80 + p.y + 80][p.x*80 + p.y] = 1;
							guigv.m.map[p.x][p.y] = 3;
						} else if (guigv.m.map[p.x][p.y] == 0 && (Map.memmap[p.x][p.y] == 3 || Map.memmap[p.x][p.y] == 2)){
							guiInfo.graph[p.x*80 + p.y][p.x*80 + p.y + 80] = 1;
							guiInfo.graph[p.x*80 + p.y + 80][p.x*80 + p.y] = 1;
							guigv.m.map[p.x][p.y] = 2;
						}
