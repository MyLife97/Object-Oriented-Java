		...
		for(i = 0; i < request.calling.size(); i++){
			...
			if(now_taxi.status == Element.Waiting){
				if(snapshot.reputation > max_rep){
					...
					min_dis = Map.distance(snapshot.x, snapshot.y, request.x_departure, request.y_departure);
				}
				else if(snapshot.reputation == max_rep &&
						Map.distance(snapshot.x, snapshot.y, request.x_departure, request.y_departure) < min_dis){
					mark = i;
					min_dis = Map.distance(snapshot.x, snapshot.y, request.x_departure, request.y_departure);
				}
			}
